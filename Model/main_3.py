...
#         print(output_result(score_lis))
        ...
        if score_lis_1[3][0] >= best_val:
            ...
            model_path = f"/workstation/jisna/saved_weights/model_3_Attention_UNet_LR_2.pth"
            torch.save(model.state_dict(), model_path)
# -
if __name__ == "__main__":
    run(all_folds, test_list,1)
    submission=pd.DataFrame(list(zip(epoch_1,loss_1)),columns=["epoch","loss"])
    submission.to_csv("model_3_Attention_UNet_LR_2_variable_length_57_features_March_15.csv",index=False)
